...
from app.crud import create_hardware_data, get_hardware_data_by_set_id
...
@router.get("/hardware/{set_id}", response_model=HardwareDataResponse)
def read_hardware_data(set_id: str, db: Session = Depends(get_db)):
    data = get_hardware_data_by_set_id(db, set_id)
    if not data:
